 ...
 *   on Swapter (both the network and the asset), so it carries a
 *   `SPECIAL_MAINNET_CASES` entry in the plugin. Swapter lists no `BSV`
 *   network at all, so mapping it to `BSV` would break the chain outright.
 */
