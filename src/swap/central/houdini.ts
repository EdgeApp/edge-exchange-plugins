import { add, ceil, floor, gt, lt, mul } from 'biggystring'
import {
  asArray,
  asBoolean,
  asDate,
...
interface HoudiniFailure {
  /**
   * The API's machine-readable error code, or `undefined` when the body is not
   * the error envelope. Classification reads THIS rather than searching the
   * raw response text: a code that also appears inside a human-readable
   * message would match a substring test on a body that means something else.
   */
  code: string | undefined
  /** What to report: the API's own reason, or the status and raw body. */
  message: string
}

/**
 * Reads a failed response body once. The message is the most specific
 * human-readable reason the error envelope carries: field messages win over
 * the top-level one, which is generic exactly when they are present. A body
 * that is not the envelope reports `endpoint`, the status and the raw text.
 */
function readHoudiniFailure(
  endpoint: string,
  status: number,
  text: string
): HoudiniFailure {
  ...
  if (apiError == null) {
    return {
      code: undefined,
      message: `Houdini ${endpoint} returned ${status}: ${text}`
    }
  }
  ...
  const reason =
    fieldMessages.length > 0 ? fieldMessages.join('; ') : apiError.message
  return { code: apiError.code, message: `HoudiniSwap: ${reason}` }
}
...
/**
 * How far a reverse quote's order may exceed the send amount the quote priced,
 * as a multiplier. A fixed-rate order restates the quote with the provider's
 * own rounding: a live BTC order asked 0.00421549 against a quoted
 * 0.004214984832 (+0.012%).
 */
const REVERSE_DEPOSIT_ALLOWANCE = '1.01'
...
export function makeHoudiniPlugin(opts: EdgeCorePluginOptions): EdgeSwapPlugin {
   ...
   */
  function cleanResponse<T>(cleaner: Cleaner<T>, raw: unknown): T {
    try {
      return cleaner(raw)
    } catch (error: unknown) {
  ...
  const fetchSwapQuoteInner = async (
    ...
    if (!quoteResponse.ok) {
      ...
      const { message } = readHoudiniFailure(
        'quotes',
    ...
    for (const candidate of inRangeQuotes.slice(0, 3)) {
      ...
      const failure = readHoudiniFailure(
        'exchange',
      ...
      if (
        ...
        failure.code !== 'STATIC_DEPOSIT_IN_USE'
      ) {
    ...
    function makeHoudiniSwapOrder(
      ...
      // instead, so the send side is Houdini's to price, and two bounds come
      // from the route the order was created on: the send amount it quoted,
      // and its published from-side ceiling, which the API enforces at order
      // creation anyway. The ceiling alone is often no bound at all (routes
      // publish `max: 9007199254740991`), so the quoted amount is what catches
      // an order that disagrees with its own quote.
      const requestedBound =
        request.quoteFor === 'from'
          ? request.nativeAmount
          : reverseDepositBound(usedQuote)
    ...
    /**
     * The most a reverse quote's order may ask the user to send: the smaller
     * of the route's quoted send amount, plus `REVERSE_DEPOSIT_ALLOWANCE` for
     * the provider's own rounding, and the route's from-side ceiling. Either
     * half the route does not publish drops out; neither leaves no bound.
     */
    function reverseDepositBound(
      quote: HoudiniQuote | undefined
    ): string | undefined {
      const bounds: string[] = []
      if (quote?.amountIn != null) {
        bounds.push(
          decimalToNativeAmount(
            fromWallet,
            mul(
              floatToDecimalString(quote.amountIn),
              REVERSE_DEPOSIT_ALLOWANCE
            ),
            request.fromTokenId,
            'up'
          )
      ...
      return bounds.reduce<string | undefined>(
        (least, bound) => (least == null || lt(bound, least) ? bound : least),
        undefined
      )
    }
  ...
  const out: EdgeSwapPlugin = {
    ...
    async fetchSwapQuote(req: EdgeSwapRequest): Promise<EdgeSwapQuote> {
      ...
      // A private request may route an asset to itself: moving it through
      // the mixer is this provider's main flow, not a user mistake. Any other
      // same-asset request is the no-op every plugin rejects, and quoting it
      // would charge exchange fees to hand the user back what they had. The
      // blocked-token checks apply either way.
      checkInvalidTokenIds(INVALID_TOKEN_IDS, request, swapInfo, {
        allowSameAsset: request.privacy === 'required'
      })
      checkWhitelistedMainnetCodes(
        MAINNET_CODE_TRANSCRIPTION,
        request,
