    ...
    if (swap.resultType !== 'OK') {
      if (
        ...
        if (amountRestriction == null) {
          // Only use binary search if error explicitly mentions amount being too low
          const isAmountTooLowError =
            (swap.error?.includes('too low') ?? false) ||
            (swap.error?.includes('below') ?? false) ||
            (swap.error?.includes('minimum') ?? false)
          ...
          if (isAmountTooLowError) {
            ...
            if (searchTokenId == null) {
              ...
              if (token == null) {
                throw new Error(
                  `Token with tokenId "${searchTokenId}" not found`
                )
              }
